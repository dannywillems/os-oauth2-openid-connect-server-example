...
let upload_user_avatar_handler myid () ((), (cropping, photo)) =
  let avatar_dir =
    List.fold_left Filename.concat
      (List.hd !Oauth_server_config.avatar_dir)
      (List.tl !Oauth_server_config.avatar_dir) in
  let%lwt avatar =
    ...
    *)
    ...
    then
      ...
      then
        ...
        in
        Oauth_server_base.App.send
          (Oauth_server_page.make_page (Os_page.content page))
      else
        let page = [ div ~a:[ a_class ["login-signup-box"] ]
...
let%shared settings_handler myid_o () () =
  let%lwt user = Oauth_server_container.get_user_data myid_o in
  let%lwt content = match user with
    | Some user -> Oauth_server_settings.settings_content user
    | None -> Lwt.return []
  in
  Oauth_server_container.page myid_o content
