...
let%shared os_footer () = Eliom_content.Html.F.(
  footer ~a:[a_class ["os-page-footer"]] [
    p [
      ...
      a ~service:Oauth_server_services.os_github_service
        [ pcdata "Ocsigen Start" ] ();
      pcdata " and uses the ";
      a ~service:Oauth_server_services.ocsigen_service
        [ pcdata "Ocsigen" ] ();
      pcdata " technology.";
    ]
  ]
...
let%shared page ?html_a ?a ?title ?head myid_o content =
  ...
  Lwt.return
    (Os_page.content ?html_a ?a ?title ?head
         ...
         ; os_footer ()
         ; Oauth_server_drawer.make ?user:me ()
         ])
