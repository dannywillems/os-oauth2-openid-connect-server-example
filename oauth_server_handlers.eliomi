...
val action_link_handler :
  ...
  string -> unit -> Oauth_server_base.App.result Lwt.t
...
(** The following functions are the handlers for the three main services defined
    in the navbar (see {!Oauth_server_container.os_header}. They are
    created with {!Oauth_server_container.page} which means that a header
...
(** This is the handler for the settings service. If the user is connected (see
    {!Oauth_server_container.get_user_data}), a settings container will be
    created. *)
